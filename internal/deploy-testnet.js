...
module.exports = function (next) {
  ...
  TokenData = '0x' + TokenData;
  ExchangeData = '0x' + ExchangeData;

  var log = console.log;

  function logTx(tx) {
    log('transaction at ' + chalk.cyan(tx));
  }
  function logI(v) {
    log(require('util').inspect(v, { colors: true, depth: 15 }));
  }
  ...
  setupWeb3().then(() => Promise.all([
    getAccounts(),
    getGasLimit(),
    getGasPrice(),
    db.sync({ force: true })
  ])).then(function (results) {
    accounts = ['0x44b5b7cf5faefa4ff01ecd9ad2f041ebd1b70c07', '0x7a9cff4720cb23dba4c66fba7c6618d1e0c5ce09'];
    primaryAccount = accounts[0];
    gasLimit = results[1];
    gasPrice = results[2]*3;
